...
int main( int argc, char** argv )
{
    ...
    for(int i=0; i < interval;i++){
		for(int j=0;j<width;j++){
			image2.at<Vec3b>(j,-i+interval-1) = image.at<Vec3b>(start+i,j);
		}
    }
	...
	if (rank == 0)
	{
		for(int i=rank+1;i<size;i++)
		{
				...
				hconcat(receive,image2,image2);
		}
		// hconcat(image2, matr, matr);
		imwrite("color_rotate.jpg", image2);
    ...
    MPI_Finalize();                                         // Wait for a keystroke in the window
    return 0;
}
